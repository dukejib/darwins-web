...
class ServiceTableSeeder extends Seeder
{
    ...
    public function run()
    {
        $img = '/img/services/img_place_holder_service.png';
        $feat = 'Featured';
        $pop = 'Popular';
        $lat = 'Latest';
        ...
        for ($i=0; $i < 6; $i++) { 
            $j = $i+1;
            $new_name = $j . ' Bed';
            Item::create(['title' =>  $new_name,'description' => $desc,'local_category_id' => 1,'slot' => $feat,'slug' => str_slug($new_name),'image' => $img, 'product' => 0 ]);
        }

        for ($i=0; $i < 6; $i++) { 
            $j = $i+1;
            $new_name = $j . ' Bed';
            Item::create(['title' =>  $new_name,'description' => $desc,'local_category_id' => 1,'slot' => $lat,'slug' => str_slug($new_name),'image' => $img , 'product' => 0 ]);
        }

        for ($i=0; $i < 6; $i++) { 
            $j = $i+1;
            $new_name = $j . ' Bed';
            Item::create(['title' =>  $new_name,'description' => $desc,'local_category_id' => 1,'slot' => $pop,'slug' => str_slug($new_name),'image' => $img , 'product' => 0 ]);
        }
        ...
        for ($i=0; $i < 6; $i++) { 
            $j = $i+1;
            $new_name = $j . ' Visa';
            Item::create(['title' =>  $new_name,'description' => $desc,'local_category_id' => 2,'slot' => $feat,'slug' => str_slug($new_name),'image' => $img , 'product' => 0 ]);
        }

        for ($i=0; $i < 6; $i++) { 
            $j = $i+1;
            $new_name = $j . ' Master';
            Item::create(['title' =>  $new_name,'description' => $desc,'local_category_id' => 2,'slot' => $lat,'slug' => str_slug($new_name),'image' => $img , 'product' => 0 ]);
        }

        for ($i=0; $i < 6; $i++) { 
            $j = $i+1;
            $new_name = $j . ' American Express';
            Item::create(['title' =>  $new_name,'description' => $desc,'local_category_id' => 2,'slot' => $pop,'slug' => str_slug($new_name),'image' => $img , 'product' => 0 ]);
        }
